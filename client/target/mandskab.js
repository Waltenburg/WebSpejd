...
class Mandskab {
    constructor() {
        ...
        this.reloadData = () => {
            ...
            const headers = null;
            sendRequest("/getData" /* Endpoints.GetMandskabData */, headers, success, fail);
        };
        this.clickedPatrol = (val, action) => {
            ...
            const targetLocationId = action === 0 /* Action.checkinToLocation */ ? this.currentLocationId : parseInt(selectedNextLocation);
            const onConfirm = () => {
                const patrolUpdate = {
                    patrolId: patrolID,
                ...
                sendRequest("/sendPatrolUpdateMandskab" /* Endpoints.MandskabSendPatrolUpdate */, header, succesReciever, onFail);
            };
            if (this.nextLocationSelector.selectedIndex === -1 && action === 1 /* Action.checkoutFromLocation */) {
            ...
            const message = `Vil du checke #${patrolNumber} ${patrolName} ${action === 0 /* Action.checkinToLocation */ ? ("ind på denne post") : ("ud imod " + nextLocationName)}?`;
            showDialog(message, [`Ja, check patrulje ${action === 0 /* Action.checkinToLocation */ ? "ind" : "ud"}`, onConfirm], ["Nej, annuller", () => { }]);
        };
        this.undoLastAction = () => {
            ...
            if (lastActionId == null)
                return;
            const onConfirm = () => {
                sendRequest(`${"/deletePatrolUpdateMandskab" /* Endpoints.MandskabDeletePatrolUpdate */}?patrolUpdateId=${lastActionId}`, null, (_status, _headers, _body) => {
        ...
        this.addPatrolsToLists = (patrolsTowardsLocation, patrolsOnLocation) => {
            ...
            const addPatrolToList = (patrol, action) => {
                ...
                if (action === 0 /* Action.checkinToLocation */)
                    this.listPatrolsTowardsLocation.appendChild(newElement);
                else
                    this.listPatrolsOnLocation.appendChild(newElement);
            };
            patrolsOnLocation.forEach(patrol => addPatrolToList(patrol, 1 /* Action.checkoutFromLocation */));
            patrolsTowardsLocation.forEach(patrol => addPatrolToList(patrol, 0 /* Action.checkinToLocation */));
