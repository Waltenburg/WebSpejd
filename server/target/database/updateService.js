...
export class UpdateService extends ServiceBase {
    ...
    isPatrolUpdateValid(newUpdate, includeRouteValidation = true, includeCurrentEqualsTargetCheck = true, isTargetFirstLocation = false, includeNoImmediateIdenticalUpdateCheck = true) {
        ...
        const routeIsValid = newUpdate.currentLocationId === newUpdate.targetLocationId || this.prepare(`SELECT 1 FROM ${"Route" /* ROUTE_TABLE.TABLE_NAME */} WHERE (${"fromLocationId" /* ROUTE_TABLE.FROM_LOCATION_ID */} = ? AND ${"toLocationId" /* ROUTE_TABLE.TO_LOCATION_ID */} = ? AND ${"is_open" /* ROUTE_TABLE.IS_OPEN */} = 1)`).get(newUpdate.currentLocationId, newUpdate.targetLocationId) != undefined;
