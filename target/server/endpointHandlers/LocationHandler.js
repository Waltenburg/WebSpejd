...
exports.getRenameLocationRow = exports.getLocationConfigTableBody = exports.getLocationConfigTable = exports.getLocationConfigTableRow = exports.deleteLocation = exports.renameLocation = exports.changeLocationStatus = exports.addLocation = void 0;
...
const html_tableBody = (locationService, locationIds) => {
    return elements.createElement("tbody", { id: "location-config-table-body", "hx-ext": "idiomorph", "hx-get": `${"/master/getLocationsTableBody"}`, "hx-target": "this", "hx-swap": "outerHTML", "hx-trigger": "every 1s", "hx-on--before-request": `if (event.detail.elt.id === this.id && ${(0, HTMLGeneral_1.isClassOnElement)((0, HTMLGeneral_1.getElementById)("location-config-table"), "renaming")}) {console.log("cancelled request"); event.preventDefault(); }` },
